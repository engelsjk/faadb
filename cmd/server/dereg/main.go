...
import (
	...
	deregrpc "github.com/engelsjk/faadb/rpc/dereg"
	"github.com/engelsjk/faadb/service/dereg"
)
...
func main() {
	...
	d, err := dereg.NewDereg(*flagDataPath, *flagDBPath)
	if err != nil {
		log.Fatal(err)
	}

	server := dereg.NewServer(d)
	twirpHandler := deregrpc.NewDeregServer(server)
